...
class ExtractionConfig {
    ...
    toApiParams(options) {
        ...
        if (this.template && this.ephemeral_template) {
            throw new errors.ExtractionConfigError('You cannot pass both parameters template and ephemeral_template. You must choose');
        }
        if (this.template) {
            params.extraction_template = this.template;
        }
        if (this.ephemeral_template) {
            params.extraction_template = 'ephemeral:' + (0, utils_js_1.urlsafe_b64encode)(JSON.stringify(this.ephemeral_template));
        }
