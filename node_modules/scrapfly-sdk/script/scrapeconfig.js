...
exports.ScrapeConfig = exports.FormatOption = exports.Format = exports.ScreenshotFlags = void 0;
...
})(ScreenshotFlags || (exports.ScreenshotFlags = ScreenshotFlags = {}));
var Format;
(function (Format) {
    /**
...
})(FormatOption || (exports.FormatOption = FormatOption = {}));
class ScrapeConfig {
    constructor(options) {
        ...
        if (options.format && !Object.values(Format).includes(options.format)) {
            throw new errors_js_1.ScrapeConfigError(`Invalid format param value: ${options.format}`);
        }
        this.format = options.format ?? this.format;
        if (options.screenshot_flags) {
            options.screenshot_flags.forEach((flag) => {
                if (!Object.values(ScreenshotFlags).includes(flag)) {
                    throw new errors_js_1.ScrapeConfigError(`Invalid screenshot_flags param value: ${flag}`);
                }
            });
        }
        ...
        if (this.body && this.data) {
            throw new errors_js_1.ScrapeConfigError('Cannot set both body and data');
        }
            ...
            if (this.data && !this.body) {
                if (!this.headers['content-type']) {
                    ...
                    if (this.headers['content-type']?.includes('application/json')) {
                        ...
                        throw new errors_js_1.ScrapeConfigError(`Content-Type "${this.headers['content-type']}" not supported, use body parameter to pass pre encoded body according to your content type`);
                    }
                }
            }
    ...
    validateOptions(options) {
        ...
        for (const key in options) {
            if (!validKeys.has(key)) {
                throw new errors_js_1.ScrapeConfigError(`Invalid option provided: ${key}`);
            }
        }
    }
    toApiParams(options) {
        ...
        if (this.render_js === true) {
            ...
            if (this.js !== undefined) {
                params.js = (0, utils_js_1.urlsafe_b64encode)(this.js);
            }
            if (this.js_scenario !== undefined) {
                params.js_scenario = (0, utils_js_1.urlsafe_b64encode)(JSON.stringify(this.js_scenario));
            }
            ...
            if (this.screenshots !== undefined) {
                ...
                if (this.screenshot_flags) {
                    logger_js_1.log.warn('Params "screenshot_flags" is ignored. Works only if screenshots is enabled');
                }
            }
            ...
            if (this.wait_for_selector !== undefined) {
                logger_js_1.log.warn('Params "wait_for_selector" is ignored. Works only if render_js is enabled');
            }
            if (this.screenshots !== undefined) {
                logger_js_1.log.warn('Params "screenshots" is ignored. Works only if render_js is enabled');
            }
            if (this.js_scenario !== undefined) {
                logger_js_1.log.warn('Params "js_scenario" is ignored. Works only if render_js is enabled');
            }
            if (this.js !== undefined) {
                logger_js_1.log.warn('Params "js" is ignored. Works only if render_js is enabled');
            }
            if (this.rendering_wait !== undefined) {
                logger_js_1.log.warn('Params "rendering_wait" is ignored. Works only if render_js is enabled');
            }
        }
        if (this.asp === true) {
            params.asp = true;
        ...
        if (this.cache === true) {
            ...
            if (this.cache_clear) {
                logger_js_1.log.warn('Params "cache_clear" is ignored. Works only if cache is enabled');
            }
            if (this.cache_ttl) {
                logger_js_1.log.warn('Params "cache_ttl" is ignored. Works only if cache is enabled');
            }
        }
        if (this.dns === true) {
            params.dns = true;
        ...
        if (this.session) {
            ...
            if (this.session_sticky_proxy) {
                logger_js_1.log.warn('Params "session_sticky_proxy" is ignored. Works only if session is enabled');
            }
        }
        if (this.debug === true) {
            params.debug = true;
