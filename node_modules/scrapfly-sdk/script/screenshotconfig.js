...
exports.ScreenshotConfig = exports.Format = exports.Options = void 0;
...
class ScreenshotConfig {
    constructor(options) {
        ...
        if (options.format && !Object.values(Format).includes(options.format)) {
            throw new errors_js_1.ScreenshotConfigError(`Invalid format param value: ${options.format}`);
        }
        ...
        if (options.options) {
            options.options.forEach((opt) => {
                if (!Object.values(Options).includes(opt)) {
                    throw new errors_js_1.ScreenshotConfigError(`Invalid options param value: ${opt}`);
                }
            });
        }
    ...
    validateOptions(options) {
        ...
        for (const key in options) {
            if (!validKeys.has(key)) {
                throw new errors_js_1.ScreenshotConfigError(`Invalid option provided: ${key}`);
            }
        }
    }
    toApiParams(options) {
...
exports.ScreenshotConfig = ScreenshotConfig;
